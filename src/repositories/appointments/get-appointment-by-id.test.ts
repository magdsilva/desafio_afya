...
describe('Repository - get-appointment-by-id', () => {
  ...
  it('propagates database failures', async () => {
    ...
    await expect(getAppointmentById(id, userId)).rejects.toBe(error)
  })

  it('handles a query without matching rows', async () => {
    queryMocked.mockResolvedValue({ rows: [], rowCount: 0 })
    await expect(getAppointmentById(id, userId)).resolves.toEqual(null)
  })
})
